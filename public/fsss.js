var app = (function () {
	...
	/* src\KeyboardEvents.html generated by Svelte v2.9.1 */
	...
	function KeyboardEvents(options) {
		this._debugName = '<KeyboardEvents>';
	...
	assign(KeyboardEvents.prototype, protoDev);
	assign(KeyboardEvents.prototype, methods);

	KeyboardEvents.prototype._checkReadOnly = function _checkReadOnly(newState) {
	};

	/* src\Screenshow.html generated by Svelte v2.9.1 */
	...
	function containerClasses({ isSugar }) {
	...
	function slidesNormalized({ slides }) {
	  slides = slides || [];
	...
	var methods$1 = {
	  ...
	  close() {
	    ...
	    // swiper
	    setTimeout(() => {
	      const { swiper: swiper$$1 } = this.get();
	      swiper$$1.destroy();
	  ...
	  handleSlideChange() {
	    const { swiper: swiper$$1, isSugar } = this.get();
	    const currentSlide = swiper$$1.slides[swiper$$1.activeIndex];
	  ...
	  onkeyup({ key }) {
	    switch (key) {
	      case 'ArrowLeft':
	        this.withSwiper(swiper$$1 => swiper$$1.slidePrev());
	        return
	      case 'ArrowRight':
	        this.withSwiper(swiper$$1 => swiper$$1.slideNext());
	        return
	      case 'Escape':
	        this.close();
	        return
	  ...
	  withSwiper(callback) {
	    const { swiper: swiper$$1, isVisible } = this.get();
	    if (isVisible) {
	      callback(swiper$$1);
	    }
	  },
	...
	function create_main_fragment$1(component, ctx) {
		...
		function onwindowresize(event) {
			...
			component.set({
				winInnerWidth: this.innerWidth,
				winInnerHeight: this.innerHeight
			});
		...
		return {
			...
			p: function update(changed, ctx) {
				...
				if (changed.slidesNormalized || changed.opts || changed.winInnerHeight || changed.winInnerWidth || changed.imagePadding) {
					each_value = ctx.slidesNormalized;
				...
				if (!current || changed.swiperId) {
					div_1.id = ctx.swiperId;
				}

				if (!current || changed.containerClasses) {
					div.className = ctx.containerClasses;
				}
			},
			...
			d: function destroy$$1(detach) {
				...
				removeListener(div_6, "click", click_handler);
	...
	function create_if_block(component, ctx) {
		...
		keyboardevents.on("keyup", function(event) {
			component.onkeyup(event);
		});

		return {
			c: function create() {
				keyboardevents._fragment.c();
			},

			m: function mount(target, anchor) {
				keyboardevents._mount(target, anchor);
				current = true;
			},
			...
			o: function outro(outrocallback) {
				...
				keyboardevents._fragment.o(outrocallback);
				current = false;
			},

			d: function destroy$$1(detach) {
				keyboardevents.destroy(detach);
			}
		};
	}
	...
	function create_if_block_3(component, ctx) {
		...
		return {
			...
			p: function update(changed, ctx) {
				if ((changed.intro) && text_value !== (text_value = ctx.intro.title)) {
	...
	function create_if_block_4(component, ctx) {
		...
		return {
			...
			p: function update(changed, ctx) {
				if ((changed.intro) && text_value !== (text_value = ctx.intro.subtitle)) {
					text.data = text_value;
				}
			},
	...
	function create_if_block_5(component, ctx) {
		...
		return {
			...
			p: function update(changed, ctx) {
				if ((changed.slidesNormalized || changed.opts || changed.winInnerHeight || changed.winInnerWidth || changed.imagePadding) && div_1_class_value !== (div_1_class_value = imageClasses(ctx.slide, { opts: ctx.opts, winInnerHeight: ctx.winInnerHeight, winInnerWidth: ctx.winInnerWidth, imagePadding: ctx.imagePadding }))) {
					div_1.className = div_1_class_value;
				}

				if ((changed.slidesNormalized) && div_1_data_background_value !== (div_1_data_background_value = ctx.slide.url)) {
					div_1.dataset.background = div_1_data_background_value;
				}
			},
	...
	function create_if_block_6(component, ctx) {
		...
		return {
			...
			p: function update(changed, ctx) {
				if ((changed.slidesNormalized) && raw_value !== (raw_value = ctx.slide.html)) {
					div.innerHTML = raw_value;
				}

				if ((changed.slidesNormalized) && div_data_fsss_sugar_value !== (div_data_fsss_sugar_value = !!ctx.slide.sugar)) {
					div.dataset.fsssSugar = div_data_fsss_sugar_value;
				}
			},
	...
	function get_each_context(ctx, list, i) {
		...
		child_ctx.slide_index = i;
		return child_ctx;
	}

	function Screenshow(options) {
		this._debugName = '<Screenshow>';
		if (!options || (!options.target && !options.root)) throw new Error("'target' is a required option");
		init(this, options);
		this._state = assign(data(), options.data);
		this._state.winInnerWidth = window.innerWidth;
		this._state.winInnerHeight = window.innerHeight;
		this._recompute({ isSugar: 1, slides: 1 }, this._state);
		if (!('isSugar' in this._state)) console.warn("<Screenshow> was created without expected data property 'isSugar'");
		if (!('slides' in this._state)) console.warn("<Screenshow> was created without expected data property 'slides'");
		if (!('isVisible' in this._state)) console.warn("<Screenshow> was created without expected data property 'isVisible'");
	...
	assign(Screenshow.prototype, protoDev);
	assign(Screenshow.prototype, methods$1);

	Screenshow.prototype._checkReadOnly = function _checkReadOnly(newState) {
		if ('winInnerWidth' in newState && !this._updatingReadonlyProperty) throw new Error("<Screenshow>: Cannot set read-only property 'winInnerWidth'");
		if ('winInnerHeight' in newState && !this._updatingReadonlyProperty) throw new Error("<Screenshow>: Cannot set read-only property 'winInnerHeight'");
		if ('containerClasses' in newState && !this._updatingReadonlyProperty) throw new Error("<Screenshow>: Cannot set read-only property 'containerClasses'");
		if ('slidesNormalized' in newState && !this._updatingReadonlyProperty) throw new Error("<Screenshow>: Cannot set read-only property 'slidesNormalized'");
	};

	Screenshow.prototype._recompute = function _recompute(changed, state) {
		if (changed.isSugar) {
			if (this._differs(state.containerClasses, (state.containerClasses = containerClasses(state)))) changed.containerClasses = true;
		}

		if (changed.slides) {
			if (this._differs(state.slidesNormalized, (state.slidesNormalized = slidesNormalized(state)))) changed.slidesNormalized = true;
		}
	};
